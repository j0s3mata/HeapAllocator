...
enum request_type { ALLOC = 1, FREE, REALLOC };
typedef struct {
 ...
 * the tests.
 */
static int test_scripts(char *script_names[], int num_script_names,
                        bool quiet) {
  int nsuccesses = 0;
  int nfailures = 0;
  ...
  *success = false;
  ...
  for (int req = 0; req < script->num_ops; req++) {
    ...
    if (script->ops[req].op == ALLOC) {
      ...
      if (!verify_payload(p, old_size, id, script, script->ops[req].lineno,
                          "freeing")) {
        return -1;
      }
  ...
  for (int id = 0; id < script->num_ids; id++) {
    if (!verify_payload(script->blocks[id].ptr, script->blocks[id].size, id,
                        script, -1, "at exit")) {
      return -1;
    }
  }
...
static void *eval_realloc(int req, size_t requested_size, script_t *script,
  ...
  if (!verify_payload(oldp, old_size, id, script, script->ops[req].lineno,
                      "pre-realloc-ing")) {
    *failptr = true;
    return NULL;
  }
  ...
  if (!verify_payload(newp,
                      (old_size < requested_size ? old_size : requested_size),
                      id, script, script->ops[req].lineno,
...
static bool verify_block(void *ptr, size_t size, script_t *script, int lineno) {
  ...
  if (ptr < heap_segment_start() || end > heap_end) {
    allocator_error(script, lineno,
                    "New block (%p:%p) not within heap segment (%p:%p)", ptr,
                    end, heap_segment_start(), heap_end);
    return false;
  }
  ...
  for (int i = 0; i < script->num_ids; i++) {
    ...
    if ((ptr >= other_start && ptr < other_end) ||
      ...
      allocator_error(script, lineno,
                      "New block (%p:%p) overlaps existing block (%p:%p)", ptr,
                      end, other_start, other_end);
      return false;
    }
  }
...
static bool verify_payload(void *ptr, size_t size, int id, script_t *script,
  ...
  for (size_t i = 0; i < size; i++) {
    if (*((unsigned char *)ptr + i) != (id & 0xFF)) {
      allocator_error(script, lineno,
                      "invalid payload data detected when %s address %p", op,
                      ptr);
      return false;
    }
  }
...
static request_t parse_script_line(char *buffer, int i, int lineno,
  ...
  char request_char;
  int nscanned =
      sscanf(buffer, " %c %d %zu", &request_char, &request.id, &request.size);
  ...
  if (!request.op || request.id < 0 || request.size > MAX_REQUEST_SIZE) {
    error(1, 0, "Line %d of script file '%s' is malformed.", lineno,
          script_name);
  }
